...
function ArticleListItem({title, sourceName, date}) {
  return <div className="Mb(10px)">
    ...
    <h3 className="Lh(1) Fz(1em) Mb(4px) Cur(p) C(dodgerblue):h"><Link to={`/articles/${title}`}>{title}</Link></h3>
  </div>;
}
...
export default class ArticleListPage extends React.Component {
  ...
  componentDidMount() {
    this.props.getArticles().then((articles) => {
  ...
  render() {
    return <div className="Mt(10px) W(1280px) Mx(a)">
      <h2 className="Fw(lr) Mb(10px)">Articles</h2>
      <div className="W(50%)">
